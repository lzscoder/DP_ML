...
"""
batch_size是在每一次更新权重之前要遍历的样本元素个数
...
class LeNetConvPoolLayer(object):
    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2)):
		...
		#每个隐层神经元（即像素）与上一层的连接数为num input feature maps * filter height * filter width。1*5*5
		...
		#maxpooling，最大子采样过程
        pooled_out = downsample.pool_2d(input=conv_out,ds=poolsize,ignore_border=True)
...
class HiddenLayer(object):
    def __init__(self, rng, input, n_in, n_out, W=None, b=None, activation=T.tanh):
         self.input = input   #类HiddenLayer的input即所传递进来的input  (500,50,4,4)
